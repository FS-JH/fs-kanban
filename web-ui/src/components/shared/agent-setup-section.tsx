...
import type { UseRuntimeSettingsAgentControllerResult } from "@/hooks/use-runtime-settings-agent-controller";
import type { UseRuntimeSettingsAgentMcpControllerResult } from "@/hooks/use-runtime-settings-agent-mcp-controller";
...
export function AgentSetupSection({
	...
	controller: UseRuntimeSettingsAgentControllerResult;
	mcpController?: UseRuntimeSettingsAgentMcpControllerResult;
	...
	const agentProviderOptions = useMemo((): SearchSelectOption[] => {
	...
	const modelPickerOptions = useMemo(
		() => buildAgentModelPickerOptions(controller.providerId, controller.providerModels),
		[controller.providerId, controller.providerModels],
	);
	const agentModelOptions = modelPickerOptions.options;
	...
	return (
		<>
			{showHeading ? <h6 className="font-semibold text-text-primary mt-4 mb-2">Agent setup</h6> : null}
			<div className="mt-2">
					...
					<SearchSelectDropdown
						...
						buttonText={
							controller.isLoadingProviderCatalog
								? "Loading providers..."
								: agentProviderOptions.find((option) => option.value === controller.providerId)?.label
						}
						...
						<SearchSelectDropdown
							...
							buttonText={
								controller.isLoadingProviderModels
									? "Loading models..."
									: agentModelOptions.find((option) => option.value === controller.modelId)?.label
							}
