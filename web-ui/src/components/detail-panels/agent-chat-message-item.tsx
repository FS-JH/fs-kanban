...
import { AgentMarkdownContent } from "@/components/detail-panels/agent-markdown-content";
...
import {
	...
	parseToolOutput,
} from "@/components/detail-panels/agent-chat-message-utils";
...
function ToolMessageBlock({ message }: { message: AgentChatMessage }): ReactElement {
...
function ReasoningMessageBlock({ message }: { message: AgentChatMessage }): ReactElement {
	return (
		<div className="w-full">
...
export function AgentChatMessageItem({ message }: { message: AgentChatMessage }): ReactElement {
	...
	if (message.role === "assistant") {
		...
		return (
			<div className="w-full px-1.5 text-sm text-text-primary">
				<AgentMarkdownContent content={normalizedAssistantContent} />
			</div>
		);
	}
