...
describe("buildAgentModelPickerOptions", () => {
	it("returns recommended models first for the cline provider", () => {
		...
		const result = buildAgentModelPickerOptions("cline", models);
	...
	it("keeps original ordering for non-cline providers", () => {
		...
		const result = buildAgentModelPickerOptions("openrouter", models);
...
describe("cline model labels", () => {
	it("formats reasoning effort labels for display", () => {
		expect(formatAgentReasoningEffortLabel("")).toBe("Default");
		expect(formatAgentReasoningEffortLabel("xhigh")).toBe("Extra high");
	});
