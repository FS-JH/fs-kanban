...
describe("AgentChatPanel", () => {
	...
	it("keeps the message list pinned to the bottom while new content streams in", async () => {
		...
		const incomingMessage: AgentChatMessage = {
			id: "assistant-2",
			role: "assistant",
	...
	it("stops auto-scroll while the user is reading older messages and re-enables it at the bottom", async () => {
		...
		const secondMessage: AgentChatMessage = {
			id: "assistant-2",
			role: "assistant",
			content: "Second reply",
			createdAt: 2,
		};
		const thirdMessage: AgentChatMessage = {
			id: "assistant-3",
			role: "assistant",
			content: "Third reply",
			createdAt: 3,
	...
	it("appends review comments into the composer draft through the panel handle", async () => {
		const panelRef = createRef<AgentChatPanelHandle>();
