...
import { AgentChatModelSelector } from "@/components/detail-panels/agent-chat-model-selector";
...
export function AgentChatComposer({
	...
	const [mentionSuggestions, setMentionSuggestions] = useState<AgentComposerCompletionSuggestion[]>([]);
	const [slashSuggestions, setSlashSuggestions] = useState<AgentComposerCompletionSuggestion[]>([]);
	...
	const activeToken = useMemo(() => detectActiveAgentComposerToken(draft, cursorIndex), [cursorIndex, draft]);
	const completionSuggestions = useMemo(() => {
		if (!activeToken) {
			return [] as AgentComposerCompletionSuggestion[];
		}
	...
	useDebouncedEffect(
		() => {
			...
			void (async () => {
				try {
					const payload = await getRuntimeTrpcClient(workspaceId).runtime.getSlashCommands.query();
	...
	const applySuggestion = useCallback(
		(suggestion: AgentComposerCompletionSuggestion, token: ActiveAgentComposerToken) => {
			const next = applyAgentComposerCompletion(draft, token, suggestion.insertText);
