...
describe("useAgentChatSession", () => {
	...
	it("appends incoming stream messages and updates existing messages by id", async () => {
		...
		const streamedMessage: AgentChatMessage = {
			id: "streamed",
			role: "assistant",
		...
		const streamedMessageUpdate: AgentChatMessage = {
			...streamedMessage,
	...
	it("updates existing message when only meta changes", async () => {
		const toolStart: AgentChatMessage = {
			id: "tool-1",
			role: "tool",
			content: "Tool: Read",
			createdAt: 2,
	...
	it("merges late-loaded history with streamed messages that arrived first", async () => {
		const deferredLoad = createDeferred<AgentChatMessage[] | null>();
		const snapshots: HookSnapshot[] = [];
