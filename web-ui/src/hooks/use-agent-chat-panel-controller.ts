// Builds the view model for the native Agent chat panel.
...
import { type AgentChatMessage, useAgentChatSession } from "@/hooks/use-agent-chat-session";
import type { RuntimeTaskImage, RuntimeTaskSessionMode, RuntimeTaskSessionSummary } from "@/runtime/types";
import { useTaskWorkspaceSnapshotValue } from "@/stores/workspace-metadata-store";

interface UseAgentChatPanelControllerInput {
	taskId: string;
...
interface UseAgentChatPanelControllerResult {
	draft: string;
...
function isAssistantLikeIncomingMessage(message: AgentChatMessage | null): boolean {
	return message?.role === "assistant" || message?.role === "reasoning";
}

function getLatestAssistantLikeIncomingMessage(messages: AgentChatMessage[] | null): AgentChatMessage | null {
...
export function useAgentChatPanelController({
	taskId,
	summary,
...
}: UseAgentChatPanelControllerInput): UseAgentChatPanelControllerResult {
	const [draft, setDraft] = useState("");
	const reviewWorkspaceSnapshot = useTaskWorkspaceSnapshotValue(taskId);
	const { messages, isSending, isCanceling, error, sendMessage, cancelTurn } = useAgentChatSession({
		taskId,
		onSendMessage,
		onCancelTurn,
		onLoadMessages,
