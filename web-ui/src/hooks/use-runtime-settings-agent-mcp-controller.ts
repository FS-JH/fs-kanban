...
import {
	fetchAgentMcpAuthStatuses,
	fetchAgentMcpSettings,
	runAgentMcpServerOAuth,
...
interface UseRuntimeSettingsAgentMcpControllerOptions {
	open: boolean;
...
export interface UseRuntimeSettingsAgentMcpControllerResult {
...
export function useRuntimeSettingsAgentMcpController(
	options: UseRuntimeSettingsAgentMcpControllerOptions,
): UseRuntimeSettingsAgentMcpControllerResult {
	...
	const reloadAuthStatuses = useCallback(async () => {
		try {
			const response = await fetchAgentMcpAuthStatuses(workspaceId);
	...
	useEffect(() => {
		...
		void Promise.all([fetchAgentMcpSettings(workspaceId), fetchAgentMcpAuthStatuses(workspaceId)])
	...
	const persistMcpSettings = useCallback(
		async (serversToSave: RuntimeClineMcpServer[]): Promise<SaveResult> => {
			const response = await saveAgentMcpSettings(workspaceId, {
	...
	const runMcpServerOauth = useCallback(
		async (serverName: string): Promise<SaveResult> => {
			...
			try {
				...
				await runAgentMcpServerOAuth(workspaceId, {
