...
import {
	...
	useRuntimeSettingsAgentMcpController,
} from "@/hooks/use-runtime-settings-agent-mcp-controller";
import type { RuntimeAgentId, RuntimeClineMcpServer, RuntimeClineMcpServerAuthStatus } from "@/runtime/types";

const fetchAgentMcpSettingsMock = vi.hoisted(() => vi.fn());
const fetchAgentMcpAuthStatusesMock = vi.hoisted(() => vi.fn());
const runAgentMcpServerOAuthMock = vi.hoisted(() => vi.fn());
const saveAgentMcpSettingsMock = vi.hoisted(() => vi.fn());

vi.mock("@/runtime/runtime-config-query", () => ({
	fetchAgentMcpAuthStatuses: fetchAgentMcpAuthStatusesMock,
	fetchAgentMcpSettings: fetchAgentMcpSettingsMock,
	runAgentMcpServerOAuth: runAgentMcpServerOAuthMock,
	saveAgentMcpSettings: saveAgentMcpSettingsMock,
}));
...
function HookHarness({
	...
	const state = useRuntimeSettingsAgentMcpController({
		open,
		workspaceId,
		selectedAgentId,
		liveAuthStatuses,
...
describe("useRuntimeSettingsAgentMcpController", () => {
	let container: HTMLDivElement;
	let root: Root;
	let previousActEnvironment: boolean | undefined;

	beforeEach(() => {
		fetchAgentMcpSettingsMock.mockReset();
		fetchAgentMcpAuthStatusesMock.mockReset();
		runAgentMcpServerOAuthMock.mockReset();
		saveAgentMcpSettingsMock.mockReset();
		fetchAgentMcpSettingsMock.mockResolvedValue({
			path: "/tmp/cline_mcp_settings.json",
			servers: [],
		});
		fetchAgentMcpAuthStatusesMock.mockResolvedValue({
			statuses: [],
		});
		runAgentMcpServerOAuthMock.mockResolvedValue({
			serverName: "linear",
			authorized: true,
			message: "Authorized",
		});
		saveAgentMcpSettingsMock.mockResolvedValue({
			path: "/tmp/cline_mcp_settings.json",
			servers: [],
		});
	...
	it("loads MCP settings when Agent is selected", async () => {
		...
		expect(fetchAgentMcpSettingsMock).toHaveBeenCalledWith("workspace-1");
		expect(fetchAgentMcpAuthStatusesMock).toHaveBeenCalledWith("workspace-1");
	...
	it("tracks unsaved MCP changes and persists them", async () => {
		...
		expect(saveAgentMcpSettingsMock).toHaveBeenCalledWith(null, {
			servers: [
				{
					name: "linear",
					disabled: false,
		...
		expect(fetchAgentMcpAuthStatusesMock).toHaveBeenCalledWith(null);
		expect(requireSnapshot(latestSnapshot).hasUnsavedChanges).toBe(false);
	});
	...
	it("runs MCP OAuth and refreshes auth statuses", async () => {
		...
		expect(runAgentMcpServerOAuthMock).toHaveBeenCalledWith("workspace-1", {
			serverName: "linear",
		});
		expect(fetchAgentMcpAuthStatusesMock).toHaveBeenCalledTimes(2);
	...
	it("applies live auth status updates while OAuth is still in progress", async () => {
		...
		runAgentMcpServerOAuthMock.mockImplementation(
			() =>
				new Promise((resolve) => {
					resolveOauth = () => {
						resolve({
	...
	it("saves unsaved MCP settings before running OAuth", async () => {
		...
		expect(saveAgentMcpSettingsMock).toHaveBeenCalledBefore(runAgentMcpServerOAuthMock);
		expect(saveAgentMcpSettingsMock).toHaveBeenCalledWith("workspace-1", {
			servers: [
				{
					name: "linear",
					disabled: false,
	...
	it("does not load MCP settings when a non-Cline agent is selected", async () => {
		...
		expect(fetchAgentMcpSettingsMock).not.toHaveBeenCalled();
		expect(fetchAgentMcpAuthStatusesMock).not.toHaveBeenCalled();
