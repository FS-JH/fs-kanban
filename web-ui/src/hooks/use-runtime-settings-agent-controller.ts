// Owns the Agent-specific settings state machine inside the settings dialog.
...
import {
	...
	runAgentProviderOauthLogin,
	saveAgentProviderSettings,
} from "@/runtime/runtime-config-query";
import { getRuntimeAgentProviderSettings } from "@/runtime/native-agent";
import type {
	RuntimeAgentId,
...
interface UseRuntimeSettingsAgentControllerOptions {
	open: boolean;
...
export interface UseRuntimeSettingsAgentControllerResult {
	providerId: string;
...
function toManagedAgentOauthProvider(value: string): RuntimeClineOauthProvider | null {
...
function normalizeBaseUrlForProvider(providerId: string, baseUrl: string | null | undefined): string {
	if (toManagedAgentOauthProvider(providerId)) {
		return "";
	}
	return baseUrl ?? "";
}
...
function getEffectiveProviderSettings(
	...
	return override ?? getRuntimeAgentProviderSettings(config);
}
...
export function useRuntimeSettingsAgentController(
	options: UseRuntimeSettingsAgentControllerOptions,
): UseRuntimeSettingsAgentControllerResult {
	...
	const configProviderSettings = getRuntimeAgentProviderSettings(config);
	...
	const normalizedProviderId = providerId.trim().toLowerCase();
	const managedOauthProvider = toManagedAgentOauthProvider(normalizedProviderId);
	...
	useEffect(() => {
		...
		void fetchAgentProviderCatalog(workspaceId)
			.then((nextCatalog) => {
				if (cancelled) {
					return;
				}
	...
	useEffect(() => {
		...
		void fetchAgentProviderModels(workspaceId, trimmedProviderId)
			.then((nextModels) => {
				if (cancelled) {
					return;
				}
	...
	const saveProviderSettingsDraft = useCallback(async (overrides?: SaveProviderSettingsOverrides): Promise<SaveResult> => {
		...
		const trimmedBaseUrl = toManagedAgentOauthProvider(trimmedProviderId)
			? null
		...
		try {
			const savedSettings = await saveAgentProviderSettings(workspaceId, {
	...
	const runOauthLogin = useCallback(async (): Promise<SaveResult> => {
		...
		try {
			const response = await runAgentProviderOauthLogin(workspaceId, {
