// Shared TRPC action hook for every native Agent chat surface.
...
export interface AgentChatActionResult {
	ok: boolean;
	message?: string;
	chatMessage?: RuntimeTaskChatMessage | null;
}

export interface SendAgentChatMessageOptions {
	mode?: RuntimeTaskSessionMode;
	images?: RuntimeTaskImage[];
}

interface UseAgentChatRuntimeActionsInput {
	currentProjectId: string | null;
	onSessionSummary?: (summary: RuntimeTaskSessionSummary) => void;
}

interface UseAgentChatRuntimeActionsResult {
	sendTaskChatMessage: (
		taskId: string,
		text: string,
...
export function useAgentChatRuntimeActions({
	currentProjectId,
	onSessionSummary,
}: UseAgentChatRuntimeActionsInput): UseAgentChatRuntimeActionsResult {
	const sendTaskChatMessage = useCallback(
		async (taskId: string, text: string, options?: SendAgentChatMessageOptions): Promise<AgentChatActionResult> => {
			if (!currentProjectId) {
	...
	const abortTaskChatTurn = useCallback(
		async (taskId: string): Promise<AgentChatActionResult> => {
			if (!currentProjectId) {
				return { ok: false, message: "No project selected." };
			}
			try {
