...
- Prefer shared runtime types, schemas, helpers, and model metadata over local redefinitions.
...
- Native Cline support has been removed from FS Kanban. Prefer the local `src/agents/`, `src/cline-sdk/` stubs, and terminal runtime layers as the source of truth.
...
- On macOS, Claude Code may be installed by the desktop app under `~/Library/Application Support/Claude/claude-code-vm/<version>/claude` without adding `claude` to `PATH`. Agent discovery and launch should treat that app-managed binary as a valid direct executable fallback instead of assuming PATH-only installation.
- Remote/Tailscale access needs two host concepts: the bind host for the local server socket and the advertised host for printed/opened URLs. Do not reuse the public/share URL for local health checks or internal tRPC calls, or `--advertise-host` / `KANBAN_RUNTIME_ADVERTISED_HOST` setups will break local CLI reachability. When debugging remote access, check both `--host` and `--advertise-host`.
- HTTPS remote access also needs two ports and two URLs: keep Kanban's internal API on local HTTP (`KANBAN_URL`, usually `http://127.0.0.1:3484`) for Foundation EA health checks and mutations, but enable Kanban's own TLS listener separately with `KANBAN_RUNTIME_TLS_CERT`, `KANBAN_RUNTIME_TLS_KEY`, and `KANBAN_RUNTIME_HTTPS_PORT`, then point the dashboard/open-link URL at that public HTTPS origin (`KANBAN_PUBLIC_URL`, for example `https://<tailscale-host>:3443`). Reusing the public HTTPS URL for Foundation EA's server-to-server calls will break local reachability.
- Do not make the live PM2 instance serve from a `dist/` tree that `npm run build` deletes in-place. That causes `ERR_MODULE_NOT_FOUND` crashes for `/dist/cli.js` mid-build and can spiral into lock-file restart churn. Keep builds non-destructive to the active `dist/` path, or stop PM2 before any destructive clean.
- Runtime-config tests are sensitive to which agent CLIs are visible on `PATH`. When asserting auto-selection behavior or whether the global config file gets materialized, explicitly control `PATH` inside the test instead of inheriting the machine environment, or local `codex`/`claude` installs will mask CI-only failures.
- Workspace-scoped runtime requests can look completely hung even when `/api/health` is fine if the global workspace index lock is held too long or if a scoped code path falls back to the path-based loaders. Do not keep `~/.config/fs-kanban/workspaces/index.json.lock` held across git probing or other slow work, and when a call site already has `workspaceId`/`repoPath` (for example `workspace.getState`, task-start hydration, project summaries), avoid routing back through `loadWorkspaceContext()` / `loadWorkspaceState()`. Holding or re-entering the index lock during `detectGitRepositoryInfo()` or similar work can make scoped tRPC calls silently stall or bounce in the UI.
...
- Runtime websocket handshakes are order-sensitive: do not register a new workspace client for incremental broadcasts before its initial `snapshot` / `aggregate_snapshot` is sent, or the UI can process `projects_updated` / `workspace_metadata_updated` out of order and render a broken intermediate state. Keep workspace metadata caches alive across short disconnects instead of dropping them immediately, or project page switches fall back to multi-second git metadata rebuilds.
- Project-wide AI board actions should reuse the existing sidebar board agent instead of inventing a second automation session path. Task-flow orchestration is already split across UI hooks and runtime/CLI code, so parallel automation entrypoints become expensive to reason about and easy to desynchronize.
- Agent approval config is now modeled by `agentApprovalMode` (`manual` / `supervised` / `full_auto`). Treat `agentAutonomousModeEnabled` as a legacy derived compatibility flag, not the source of truth for new code.
- Hook-driven `to_review` transitions are overloaded: some mean genuine completion, others mean permission prompts or attention stops. Never broadcast `task_ready_for_review` or “ready” UX from the transition alone; gate it on hook metadata such as `notificationType`, `hookEventName`, and `toolInputSummary` so permission prompts stay classified as waiting.
